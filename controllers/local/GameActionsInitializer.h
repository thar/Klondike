            ...
            void addKlondikeCommandGameActions(LocalPlayerController &player)
            {
                player.addGameAction(
                        std::make_shared<KlondikeCommandGameAction>("Move from Stock to Waist", game_.getCommand(0),
                                                                    player.getUndoStack(), player.getRedoStack()));
                player.addGameAction(
                        std::make_shared<KlondikeCommandGameAction>("Move from Waist to Stock", game_.getCommand(1),
                                                                    player.getUndoStack(), player.getRedoStack()));
                player.addGameAction(std::make_shared<KlondikeCommandGameAction>("Move from Waist to Foundation",
                                                                                 game_.getCommand(2),
                                                                                 player.getUndoStack(),
                                                                                 player.getRedoStack()));
                player.addGameAction(
                        std::make_shared<KlondikeCommandGameAction>("Move from Waist to Tableau", game_.getCommand(3),
                                                                    player.getUndoStack(), player.getRedoStack()));
                player.addGameAction(std::make_shared<KlondikeCommandGameAction>("Move from Tableau to Foundation",
                                                                                 game_.getCommand(4),
                                                                                 player.getUndoStack(),
                                                                                 player.getRedoStack()));
                player.addGameAction(
                        std::make_shared<KlondikeCommandGameAction>("Move from Tableau to Tableau", game_.getCommand(5),
                                                                    player.getUndoStack(), player.getRedoStack()));
                player.addGameAction(std::make_shared<KlondikeCommandGameAction>("Move from Foundation to Tableau",
                                                                                 game_.getCommand(6),
                                                                                 player.getUndoStack(),
                                                                                 player.getRedoStack()));
            }
